...
int wmain(int argc, wchar_t **argv) {
    if (argc != 4 && argc != 5) { fputs("Expected runtime, classpath, input path and optional fast-evaluate\n", stderr); return 2; }
    if (argc == 5 && wcscmp(argv[4], L"fast-evaluate") != 0) return 2;
    ...
    JavaVMOption options[] = {
        ...
        /* Disposable evaluators favor startup latency over peak long-running throughput. */
        {"-XX:TieredStopAtLevel=1", NULL}
    };
    jint option_count = (jint)(sizeof(options) / sizeof(options[0])) - (argc == 5 ? 0 : 1);
    JavaVMInitArgs init = {JNI_VERSION_1_8, option_count, options, JNI_FALSE};
    JavaVM *vm = NULL;
    JNIEnv *env = NULL;
