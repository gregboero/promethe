...
reporter = 'scripts/report-kotlin-amortization.py'
assert all(sha(root / p) == h for p, h in before['sourceSha256'].items() if p != reporter)
...
with sqlite3.connect(f'file:{(live / "campaign-budget.sqlite").as_posix()}?mode=ro', uri=True) as db:
    for result in results:
        ...
        for i, p in enumerate((source / result['session']).rglob('*.md')):
            ...
            # CopyFile2 needs an extended absolute path for long generated skill filenames.
            source_path = '\\\\?\\' + str(p.resolve()) if os.name == 'nt' else p
...
summary = dict(batch=source.name, plannedRuns=3, recordedRuns=len(results),
               ...
               reporterSha256AtFreeze=before['sourceSha256'][reporter], reporterSha256AtReconciliation=sha(root / reporter),
               reporterNote='Post-run Windows long-path archive fix; experiment Kotlin sources still match their frozen hashes',
