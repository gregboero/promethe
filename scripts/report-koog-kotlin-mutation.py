"""Reconcile the bounded optional Kotlin mutation pilot without rewriting earlier cohorts."""
import argparse
import hashlib
import json
...
assert source.parent == live.resolve() and source.name.startswith('koog-kotlin-')
read = lambda p: json.loads(p.read_text(encoding='utf-8'))
protocol, results, preflight = [read(source / f'{name}.json') for name in ('protocol', 'results', 'preflight')]
before = read(root / 'build/reports/harness-koog-kotlin/before.json')
...
for result in results:
    ...
    for receipt in calls:
        ...
        if phase['agentStep'] is not None:
            assert receipt['toolDefinitions'] == (7 if result['mode'] == 'free' else 1)
...
summary = dict(batch=source.name, plannedRuns=2, recordedRuns=len(results), completeCohort=len(results)==2,
...
target = root / 'docs/reports/harness-koog-kotlin-mutation-data-2026-09-07' / source.name
target.mkdir(parents=True, exist_ok=True)
for path in source.glob('*.json'): shutil.copy2(path, target / path.name)
for name in ('harness-koog-kotlin-final-validation.log', 'harness-koog-kotlin-live.log'):
    shutil.copy2(root / 'build/reports' / name, target / name)
shutil.copy2(root / 'shared/build/test-results/harnessLiveTest/TEST-dev.promethe.core.HarnessKoogMutationLiveTest.xml', target / 'live-test.xml')
