...
if args.freeze:
    ...
    summary = read(work / 'native-verified/summary.json')
    ...
    assert [d['choice'] for d in read(work / 'native-verified/decisions.json')] == ['CREATE', 'REUSE']
    counts = {}
    ...
    for name in ('summary.json', 'decisions.json', 'library.json', 'compatibility.txt', 'preflight.json'):
        shutil.copy2(work / 'native-verified' / name, archive / name)
    ...
    uri_check = archive / 'initial-uri-check'
    uri_check.mkdir(exist_ok=True)
    for name in ('uri-check-validation.log', 'uri-check-native-test.xml', 'frozen-before-uri-check.json', 'HarnessAdaptationNativeTest-before-uri-check.kt', 'report-before-uri-check.py', 'uri-check-events.json'):
        shutil.copy2(work / name, uri_check / name)
    write(uri_check / 'interpretation.json', dict(transformedObservations=1, cause='Native candidate evaluation and activation passed. Test expected artifact://hash instead of actual artifact://sha256/hash; corrected assertion. Runtime code unchanged.'))
