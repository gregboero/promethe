...
class HarnessDecisionLiveTest {
    @Test fun `compare free mutation against original and fixed processor`() =
        runBlocking {
            ...
            val diagnosticPilot = System.getenv("PROMETHE_HARNESS_DIAGNOSTIC_PILOT") == "true"
            require(!exposure || kotlin) { "Exposure experiment requires Kotlin" }
            require(!diagnosticPilot || (exposure && kotlin)) { "Diagnostic pilot requires Kotlin exposure protocol" }
            val repetitions = if (diagnosticPilot) {
                1
            } else if (kotlin) {
                3
            } else {
            ...
            val seedBase = if (diagnosticPilot) {
                199371
            } else if (exposure) {
                99371
            } else {
            ...
            val resume = System.getenv("PROMETHE_HARNESS_RESUME_COHORT")
            require(!diagnosticPilot || resume == null) { "Diagnostic pilot requires a fresh cohort; never retry historical failures" }
            if (resume != null) require(exposure && Regex("kotlin-exposure-[0-9a-f-]{36}").matches(resume)) { "Resume requires an exact exposure cohort name" }
            val batch = resume ?: "${if (exposure) {
                "kotlin-exposure"
            } else if (kotlin) {
                "kotlin-decision"
            } else {
            ...
            if (resume != null) check(Files.isRegularFile(output.resolve(batch).resolve("results.json"))) { "Resume cannot create a new cohort" }
            ...
            val protocol = buildJsonObject {
                ...
                if (diagnosticPilot) {
                    put("diagnosticPilot", true)
                    put("purpose", "six-run response-diagnostics smoke test; not a quality benchmark")
                }
                if (exposure) {
                    put("completionContract", "all-distinct-pages-and-integer-array-arity-v1")
            ...
            if (resume == null) {
                Files.writeString(directory.resolve("protocol.json"), protocol.toString())
            } else {
                check(Json.parseToJsonElement(Files.readString(directory.resolve("protocol.json"))) == protocol) { "Cannot change the design when resuming a cohort" }
            }
            ...
            val results = if (resume == null) mutableListOf<JsonElement>() else Json.parseToJsonElement(Files.readString(directory.resolve("results.json"))).jsonArray.toMutableList()
            val completedSessions = results.map { it.jsonObject.getValue("session").jsonPrimitive.content }.toSet()
            check(completedSessions.size == results.size) { "Duplicate recorded sessions" }
                    ...
                    for (repeat in 1..repetitions) {
                        for ((familyIndex, family) in families.withIndex()) {
                            ...
                            for (mode in modes) {
                                ...
                                // Recorded failures are evidence too: never retry or replace an existing row.
                                if (session in completedSessions) continue
                                ...
                                // Preserve the byte-only rule preregistered for this older protocol.
                                val exposurePolicy = HarnessExposurePolicy(estimatedExposureBytesPerCall = 0)
                                ...
                                try {
                                    ...
                                    val validator = if (exposure) {
                                        AgentCompletionValidator { candidate ->
                                            final = candidate
                                            HarnessTaskCompletion.requireComplete(candidate, readPages, pageCount)
                                        }
                                    } else {
                                        null
                                    }
                                    val agent = AIAgent(config, database, adapter, ProfileManager(null), executor, SkillLoader(getFileSystem(), skills), TrajectoryEvaluator(adapter, config), SkillWriter(getFileSystem(), skills), maxIterations = iterationLimit, completionValidator = validator)
                                    ...
                                    failureCode = (error as? AgentExecutionException)?.code
                                } finally {
                                ...
                                val record = buildJsonObject {
                                    ...
                                    if (exposure) {
                                        put("completion", Json.encodeToJsonElement(HarnessTaskCompletion.assess(final, readPages, pageCount)))
                                        put("readPages", JsonArray(readPages.map(::JsonPrimitive)))
                                    }
