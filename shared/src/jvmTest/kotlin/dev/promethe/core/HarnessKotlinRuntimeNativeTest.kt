...
class HarnessKotlinRuntimeNativeTest {
    private fun env(name: String) = Path.of(requireNotNull(System.getenv(name)))

    @Test fun `profile and compare disposable runtime preparation`() =
        runBlocking<Unit> {
            assumeTrue(System.getenv("PROMETHE_HARNESS_RUNTIME_NATIVE") == "true")
            ...
            Files.writeString(
                ...
                buildJsonObject {
                    ...
                    put("baseline", "current disposable worker, full preparation per call")
                    put("optimized", "per-session runtime files, source and snapshot SHA-256 checked every call; fresh process per invocation")
            ...
            NativeSandboxManager(env("PROMETHE_HARNESS_HELPER"), JvmSandboxHelperProcessFactory).use { sandbox ->
                ...
                repeat(if (System.getenv("PROMETHE_HARNESS_RUNTIME_PROFILE") == "true") 1 else 3) { repetition ->
                    for (optimized in if (System.getenv("PROMETHE_HARNESS_RUNTIME_PROFILE") == "true") {
                        listOf(false)
                        ...
                        val runner = HarnessKotlinRunner(sandbox, env("PROMETHE_HARNESS_KOTLIN_DIST"), env("PROMETHE_HARNESS_JAVA_RUNTIME"), env("PROMETHE_HARNESS_SCRATCH"), metrics = { metrics.add(it) }, timing = { timings.add(it) }, reusePreparedRuntime = optimized)
                        ...
                        try {
                            ...
                            rows.add(
                                buildJsonObject {
                                    put("mode", mode)
                                    put("timings", Json.encodeToJsonElement(timings))
                                    ...
                                    put("runtimeCleaned", runner.preparedRuntimeEntries() == 0)
                ...
                if (System.getenv("PROMETHE_HARNESS_RUNTIME_PROFILE") == "true") return@use
                val metrics = mutableListOf<KotlinHarnessMetrics>()
                val runner = HarnessKotlinRunner(sandbox, env("PROMETHE_HARNESS_KOTLIN_DIST"), env("PROMETHE_HARNESS_JAVA_RUNTIME"), env("PROMETHE_HARNESS_SCRATCH"), metrics = { metrics.add(it) })
