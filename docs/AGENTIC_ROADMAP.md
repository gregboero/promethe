...
6. **PARTIEL** — cœur MCP 2026-07-28 dual-era. Le client Streamable HTTP sonde `server/discover`, envoie les métadonnées et headers routables par requête, puis ne retombe sur le lifecycle 2025-11-25 qu'après détection d'un serveur legacy. Le serveur valide les enveloppes stateless, publie des catalogues déterministes et cacheables, et conserve le lifecycle historique sur la même route sans annoncer Tasks aux clients modernes. Il reste MRTR, l'extension Tasks réellement connectée, les abonnements et les schémas 2020-12 riches issus des contrats typés.
