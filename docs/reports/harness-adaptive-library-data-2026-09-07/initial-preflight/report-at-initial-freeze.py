...
if args.freeze:
    ...
    summary = read(work / 'native/summary.json')
    ...
    assert [d['choice'] for d in read(work / 'native/decisions.json')] == ['CREATE', 'REUSE']
    counts = {}
    ...
    for name in ('summary.json', 'decisions.json', 'library.json', 'compatibility.txt'):
        shutil.copy2(work / 'native' / name, archive / name)
