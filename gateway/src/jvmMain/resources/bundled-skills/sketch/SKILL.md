...
- User wants a diagram — use an architecture-diagram skill
...
Write variants using `file_write(path=..., content=...)`. Open them in a browser to verify. If it looks broken, fix it before showing the user.

### 4. Head-to-Head

Present the variants to the user with a brief summary of each stance. Let the user pick or combine. Don't advocate for one — that's their call.

### 5. After the Pick

When the user picks a direction:
- Note what they liked (use this as the constraint for future iterations)
- Offer one round of iteration on the winner
- Don't keep the losers unless asked — they're throwaway

## Rules

1. Always produce at least 2 variants. If you only have one idea, push harder.
2. Each variant must open in a browser and look finished (not broken/half-rendered).
3. Use `file_write(path=..., content=...)` to create HTML files.
4. Never produce shippable code — this is exploration, not implementation.
5. Realistic content only — no "Lorem ipsum", no "User Name".
