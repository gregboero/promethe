...
1. Read user content: inline text, `file_read(path=...)` for files, or `http_fetch(url=..., method="GET")` for URLs
2. Choose layout + style (user preference or auto-select from recommended combinations)
3. Generate the infographic as an HTML/SVG file
4. Save: `file_write(path="infographic.html", content=...)`
5. Present to user with path and preview instructions
